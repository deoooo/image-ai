...
export function TeamAdminManager({ token, user, onLogout }: { token: string; user: TeamAdmin; onLogout: () => void }) {
  ...
  async function createMember(event: FormEvent) {
    ...
    try {
      ...
      setMembers((items) => [data.user, ...items]); setUsername(""); setPassword(""); setDailyLimit(0); setMessage("Member created.");
  ...
  async function saveLimit(member: Member) {
    const value = Number(limits[member.id]); if (!Number.isFinite(value) || value < 0) return setMessage("Daily limit must be non-negative.");
    ...
    try {
      ...
      setMembers((items) => items.map((item) => item.id === member.id ? data.user : item)); setLimits((v) => ({ ...v, [member.id]: "" })); setMessage("Daily limit updated.");
  ...
  return <main className="min-h-screen bg-gray-50 p-4 md:p-8"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="rounded-xl border bg-white p-5"><h2 className="mb-4 flex items-center gap-2 font-semibold"><Users className="h-4 w-4" /> Members ({actualMembers.length})</h2><div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead><tr className="border-b text-gray-500"><th className="py-3">Username</th><th>Spent today</th><th>Daily limit</th><th>New limit</th><th /></tr></thead><tbody>{actualMembers.map((member) => <tr key={member.id} className="border-b"><td className="py-3 font-medium">{member.username}</td><td>{member.dailySpent}</td><td>{member.dailyLimit ?? 0}</td><td><input aria-label={`New daily limit for ${member.username}`} type="number" min="0" step="0.001" value={limits[member.id] || ""} onChange={(e) => setLimits((v) => ({ ...v, [member.id]: e.target.value }))} placeholder={String(member.dailyLimit ?? 0)} className="h-9 w-32 rounded-lg border px-2" /></td><td className="text-right"><button onClick={() => void saveLimit(member)} disabled={saving} className="rounded-lg border px-3 py-2 font-medium disabled:opacity-40">Save</button></td></tr>)}</tbody></table></div></section>
  </div></main>;
}
